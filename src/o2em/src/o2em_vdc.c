...
#ifdef TARGET_GNW
/* Frame buffer only — no bmpcache (scanlines disabled on G&W; saves ~85 KiB). */
...
#else
   /* bmp in RAM_EMU BSS; collision buffer in DTCM (not AHB / not ITCM data). */
   bmp = &local_bmp;
   bmp->line  = bmp_buff;
   bmp->w     = BMPW;
   bmp->h     = BMPH;
...
#else
	col = dtc_malloc(BMPW * BMPH);
	if (!col || col == (void *)0xffffffff)
		col = ram_malloc(BMPW * BMPH);
#endif
